<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="JJDay.aspx.cs" Inherits="direct_demo.demo1.manage.JJDay" %>
...
<asp:Repeater ID="rptlist" runat="server">
  <ItemTemplate>
          <tr>
                                     ...
                                     <td><a href="MemberView.aspx?username=<%#Eval("username").ToString()%>"><%#Eval("username").ToString()%></a></td>
                                    <%--<td title="市场奖"><%#float.Parse(Eval("jj2").ToString()).ToString("f2")%></td>--%>
                                    <td title="管理奖"><%#float.Parse(Eval("jj5").ToString()).ToString("f2")%></td>
                                    <%--<td title="利息"><%#float.Parse(Eval("jj7").ToString()).ToString("f2")%></td>
                                    <td title="收入"><%#float.Parse(Eval("jj").ToString()).ToString("f2")%></td>--%>
                                    <td><a href="jjDayDetail.aspx?addtime=<%#Server.UrlEncode(Eval("addtime").ToString())%>&bianhao=<%#Eval("username").ToString() %>">查看详细</a></td>
          </tr>
  </ItemTemplate>
</asp:Repeater>
