<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MessageList2.aspx.cs" Inherits="direct_demo.demo1.manage.MessageList2" %>
          ...
          <tr>
            <td width="46%" valign="middle"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td width="95%" class="STYLE1"><span class="STYLE3">你当前的位置</span>：[站内短信]</td>
              </tr>
            </table></td>
...
<asp:Repeater ID="rptlist" runat="server">
   <ItemTemplate>
          <tr>
            <td width="40">
              <input type="checkbox" id="cb<%#Eval("MId").ToString() %>" name="Cbs" value="<%#Eval("MId").ToString() %>"/>
            </td>
            <td height="20" width="40"><%#Container.ItemIndex+1 %></td>
            <td width="120"><%#Eval("MFrom").ToString()%></td>
            <td width="120"><%#Eval("MTo").ToString()%></td>
            <td width="120"><%#Eval("stype").ToString()%></td>
            <td style="word-break:break-all;"><%#Eval("MContent").ToString() %></td>
            <td width="50">
            
            <span style=" <%#Eval("pic1").ToString() == "" ? "display:none;" : ""%>">
            <a href="<%#Eval("pic1").ToString() %>" target="_blank">查看</a> 
            </span>
            </td>
            <td width="120"><%#Eval("MAddTime") %></td>
            <td width="80">
                <%--<asp:HyperLink ID="HyperLink1" NavigateUrl='<%#"messageview.aspx?id="+Eval("MId") %>' runat="server">查看</asp:HyperLink>--%>
                <span style="<%#Eval("MTo").ToString().Trim() != "admin" ? "display:none;" : ""%>">  <asp:HyperLink ID="HyperLink2" NavigateUrl='<%#"messageAdd.aspx?id="+Eval("MId") %>' runat="server">回复</asp:HyperLink></span>  
                ...
                <asp:LinkButton ID="LinkButton1" OnCommand="Oprerate_Command" CommandName="del" CommandArgument='<%#Eval("MId") %>' runat="server"  OnClientClick="return ifConfirm()">删除</asp:LinkButton>  
            </td>
          </tr>
          <%#getMessage(Int32.Parse(Eval("MId").ToString()))%>
          <%--<tr>
